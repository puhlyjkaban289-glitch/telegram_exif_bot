...
def apply_adversarial_bim(img: Image.Image, epsilon: float = 0.04, alpha: float = 0.01, num_iter: int = 8) -> Image.Image:
    """
    Применяет Basic Iterative Method (BIM / I-FGSM) — итеративную adversarial-атаку.
    Делает изображение более устойчивым к нейросетевым детекторам и perceptual-hash.
    Использует лёгкую линейную модель (быстро, без тяжёлых весов).
    Требует: torch + numpy
    """
    try:
        import torch
    ...
    # Для скорости работаем на уменьшенной копии (макс. 320 px по длинной стороне)
    orig_w, orig_h = img.size
    max_side = 320
    scale = min(1.0, max_side / max(orig_w, orig_h))
    work_w = max(32, int(orig_w * scale))
    work_h = max(32, int(orig_h * scale))
    ...
    # Лёгкая линейная модель (случайные веса)
    ...
    # Возвращаем в PIL и масштабируем обратно
    adv_arr = (adv.squeeze(0).permute(1, 2, 0).numpy() * 255.0).clip(0, 255).astype(np.uint8)
    adv_pil = Image.fromarray(adv_arr)

    if (work_w, work_h) != (orig_w, orig_h):
        adv_pil = adv_pil.resize((orig_w, orig_h), Image.LANCZOS)
...
def process_photo(
    ...
    if adversarial:
        img = apply_adversarial_bim(img, epsilon=0.04, alpha=0.01, num_iter=8)
